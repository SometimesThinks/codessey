...
- **실습 일자:** 2026-09-08
- **목적:** AWS 네트워크 직접 구성부터 EC2 웹 서비스 배포·외부 접속 검증까지의 수행 과정 공유
- **문서 범위:** 기본 Nginx 배포 당시 기록, 이후 Docker·HTTPS 전환 과정은 [Docker 기록](../04_Docker/docker.md)과 [HTTPS 기록](../03_HTTPS/https.md)으로 별도 정리
- **증빙 구성:** 섹션별·촬영 순서별 구축·검증 캡처 30장과 개별 설명, 오류 캡처의 별도 보고서 분리
- **작성 기준:** 실제 캡처 우선, 대화로 확인된 진행 상황 보완, 입력 중 화면과 완료 결과의 구분
- **무료 도메인 서비스:** [DuckDNS](https://www.duckdns.org/)의 무료 서브도메인 `codyssey.duckdns.org` 사용 및 EC2 퍼블릭 IP 연결, [HTTPS 적용 기록](../03_HTTPS/https.md) 참조
...
- **확인 포인트:** 계획의 Ubuntu 24.04와 구분되는 실제 선택값, SSH 로그인 배너에서도 26.04 LTS 확인
